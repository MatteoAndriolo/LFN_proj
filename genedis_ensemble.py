...
def get_embedding(vectors: dict):
    """
    Map embedding dictionary to np matrix n x d

    Parameters:
    ------------
    vector : dict
        dictionary containing embedding for each node
...
def get_embedding_lap(vectors: dict):
    """
    Map laplacian embedding dictionary to np matrix n x 128 
    ...
    ------------
    matrix: np.ndarray
        dimension n x 128, default vale is 0
    """
...
def processEmb(oldfile,newfile):
    """
    Append embedding from oldfile to newfile
...
def Net2edgelist(gene_disease_matrix_net):
    """
    Save in gene_desease.txt edge list obtained from gene_disease_matrix_net

    Parameter:
    ---
    gene_disease_matrix_net: int[[]]
        matrix containing containing 1 if gene associated with diseasre, 0 otherwise
    """    
